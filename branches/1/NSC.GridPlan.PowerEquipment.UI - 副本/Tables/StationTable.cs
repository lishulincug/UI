...
namespace NSC.GridPlan.PowerEquipment.UI.Tables
{
    public class StationTable
    {
        ...
        public static Dictionary<string, string> GetFieldName(string m_SubType)
        {
            ...
            FieldName.Add("基本信息", "名称,简称,变电站类型,电压等级(kV),分区名,供电公司,所属区域,调度关系,运维单位,建设进度,核准进度,投产年,供电区域,行政区域");
            if (m_SubType.Contains("交流"))
            {
                FieldName.Add("升压变信息", "主变组数(终期),主变组数(已建),出线(已建),出线(规划),出线方向,开关(遮断能力),升压变信息");
            }
            else
            {
        ...
        public static Dictionary<string, object> GetValueInfo(SubStaObjects mSubSta)
        {
            ...
            //交流变电站主变表
            FieldValue.Add("升压变信息", mSubSta.SubGUSCollection);
            //变电站母线表、变电站母线负荷信息表、变电站电容电抗器表
            FieldValue.Add("母线信息", mSubSta.SubBusCollection);
            //机组信息
            FieldValue.Add("机组信息", mSubSta.SubBusCollection);
            //串补表
            FieldValue.Add("串补站", "");
            FieldValue.Add("安装线路", "");
            FieldValue.Add("安装地点", "");
        ...
        public static Dictionary<string, ComEditType> GetFieldCom()
        {
            ...
            //交流变电站主变表
            FieldLink.Add("升压变信息", ComEditType.数据集合);
            //变电站母线表、变电站母线负荷信息表、变电站电容电抗器表
