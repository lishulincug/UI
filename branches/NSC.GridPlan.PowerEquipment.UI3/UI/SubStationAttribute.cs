...
namespace NSC.GridPlan.PowerEquipment.UI
{
    public partial class SubStationAttribute : Form
    {
        ...
        private DevExpress.XtraVerticalGrid.Rows.CategoryRow tabCategoryRow;
        /// <summary>
        /// 行控件
        /// </summary>
        ...
        private void InitalGridControlByMdb(DevExpress.XtraVerticalGrid.VGridControl vGridControlx, DataSet Ds, string tableName, int iCount)
        {
            ...
            vGridControlx.SaveLayoutToXml(Application.StartupPath + @"\Model\GV_SUBSTATION\" + vGridControlx.Tag.ToString() + ".xml");
            //vGridControlx.RestoreLayoutFromXml(Application.StartupPath + @"\Model\GV_SUBSTATION\" + vGridControlx.Tag.ToString() + ".xml");
        }
        /// <summary>
        /// 多表合成界面
        /// </summary>
        /// <param name="vGridControlx"></param>
        /// <param name="Ds"></param>
        /// <param name="tableName"></param>
        /// <param name="iCount"></param>
        private void AllInitalGridControlByMdb(DevExpress.XtraVerticalGrid.VGridControl vGridControlx, DataSet Ds, string tableName, int iCount)
        {
            ...
            strGroupName1.Add("必填");
            strGroupName1.Add("选填");
            IList<string> allColumnName=new List<string>();
            for (int j = 0; j < strGrouptableName.Count; j++)
            {
                if (Ds.Tables[strGrouptableName[j]] != null)
                {
                    this.tabCategoryRow = new DevExpress.XtraVerticalGrid.Rows.CategoryRow();
                    this.tabCategoryRow.Properties.Caption = strGrouptableName[j].Replace("[", "").Replace("]", "");
                    this.tabCategoryRow.Name = strGrouptableName[j].Replace("[", "").Replace("]", "");
                    vGridControlx.Rows.AddRange(new DevExpress.XtraVerticalGrid.Rows.BaseRow[] {
                this.tabCategoryRow});
                    //vGridControlx.Rows.Clear();
                    this.nCategoryRow = new DevExpress.XtraVerticalGrid.Rows.CategoryRow();
                    this.nCategoryRow.Properties.Caption = strGroupName1[0];
                    //nCategoryRow.Name = strGroupName1[0]  ;
                    this.tabCategoryRow.ChildRows.AddRange(new DevExpress.XtraVerticalGrid.Rows.BaseRow[] {
                this.nCategoryRow});
                    this.nCategoryRow.Grid.Dock = DockStyle.Fill;

                    this.mCategoryRow = new DevExpress.XtraVerticalGrid.Rows.CategoryRow();
                    this.mCategoryRow.Properties.Caption = strGroupName1[1];
                    //mCategoryRow.Name = strGrouptableName[j].Replace("[", "").Replace("]", "") + strGroupName1[1];
                    this.tabCategoryRow.ChildRows.AddRange(new DevExpress.XtraVerticalGrid.Rows.BaseRow[] {
                this.mCategoryRow});
                    this.mCategoryRow.Grid.Dock = DockStyle.Fill;
                    //循环填充表格
                    for (int i = 0; i < Ds.Tables[strGrouptableName[j]].Columns.Count; i++)
                    {
                        ...
                        //设置row属性
                        if (!allColumnName.Contains(Ds.Tables[strGrouptableName[j]].Columns[i].ColumnName))
                        {
                            allColumnName.Add(Ds.Tables[strGrouptableName[j]].Columns[i].ColumnName);
                            this.mEditorRow = new DevExpress.XtraVerticalGrid.Rows.EditorRow();
                            this.mEditorRow.Name = Ds.Tables[strGrouptableName[j]].Columns[i].ColumnName;
                            this.mEditorRow.Properties.FieldName = Ds.Tables[strGrouptableName[j]].Columns[i].ColumnName;
                            this.mEditorRow.Properties.UnboundType = DevExpress.Data.UnboundColumnType.Object; //Ds.Tables[strGrouptableName[j]].Columns[i].DataType; //
                            this.mEditorRow.Height = 19;
                            this.mEditorRow.Appearance.Options.UseForeColor = true;
            ...
            // vGridControlx 行宽自动调整
            //RecalcWidth(vGridControlx);
            //vGridControlx.SaveLayoutToXml(Application.StartupPath + @"\Model\GV_SUBSTATION\All1" +  ".xml");
            //vGridControlx.RestoreLayoutFromXml(Application.StartupPath + @"\Model\GV_SUBSTATION\" + vGridControlx.Tag.ToString() + ".xml");
        }
        protected void InitNWindData()
        {
            ...
            if (DBFileName != string.Empty)
            {
                ...
                for (int i = 0; i < strGrouptableName.Count; i++)
                {
                    ...
                    vGridControl2.DataSource = ds.Tables[strGrouptableName[i]];
                }
                //ds.Tables.AddRange();
    ...
    /// <summary>
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            ...
            //EquipmentInfoShow vGridControli1 = EquipmentInfoShow.vGridControl1;
        ...
        {
            int mBusNum = 1;// BaseFunction.GetCode(textBus.Text);//获取主变台数                      
            ...
            if (DBFileName != string.Empty)
            {
                ...
                //InitalGridControlByMdb(vGridControl1, ds, TableGV_SUBSTATION_BUSBAR, mBusNum);
        ...
        private void Trans_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (DBFileName != string.Empty)
            {
                ...
                //InitalGridControlByMdb(vGridDdtial, ds, TableGV_AC_SUBSTATION_PT, mTransNum);
        ...
        {
            int mPcrNum = 1;// BaseFunction.GetCode(textPcr.Text);//获取主变台数                      
            ...
            if (DBFileName != string.Empty)
            {
                ...
                //InitalGridControlByMdb(vGridControl3, ds, TableGV_SUBSTATION_PCR, mPcrNum);
        ...
        private void toolStripTextTrans_Click(object sender, EventArgs e)
        {
            //注：初始化默认主变为3台
            int mTransNum = BaseFunction.GetCode(toolStripTextTrans.Text);//获取主变台数                           
        ...
        private void toolStripTextBoxBus_Click(object sender, EventArgs e)
        {
            int mBusNum = BaseFunction.GetCode(toolStripTextBoxBus.Text);//获取台数                      
            DBFileName = DevExpress.Utils.FilesHelper.FindingFileName(Application.StartupPath, "\\湖北省2012年地理接线图.mdb");
        ...
        private void toolStripTextBoxPCR_Click(object sender, EventArgs e)
        {
            int mPcrNum = BaseFunction.GetCode(toolStripTextBoxPCR.Text);//获取主变台数                      
            if (DBFileName != string.Empty & mPcrNum > 0)
            {
                string TableGV_SUBSTATION_PCR = "[GV_SUBSTATION_PCR]";
                DataSet ds = new DataSet();
                System.Data.OleDb.OleDbDataAdapter oleDbDataAdapter = new System.Data.OleDb.OleDbDataAdapter("SELECT TOP " + mPcrNum.ToString() + " * FROM " + TableGV_SUBSTATION_PCR, "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + DBFileName);
